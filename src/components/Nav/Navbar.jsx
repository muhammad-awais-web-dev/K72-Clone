  ...
  return (
    <nav className="navbar h-fit w-screen  relative flex flex-col justify-center items-center">
      ...
      <div className="topBar">
        <div
          onClick={() => navigate("/")}
          className=" cursor-pointer absolute p-5 border-2 border-white -mt-2 -ml-2 top-0 left-0"
        >
          <svg
          ...
          {navItems.map((item, index) => (
            <>
              <Link
                to={item.link}
                key={index}
                className="actualLinks relative w-full h-1/4 flex flex-col justify-center items-start overflow-hidden border-y -mt-1 bg-black border-white text-white"
              >
